...
print(f"Loading model and tokenizer... tooks {time.time() - start:.2f} seconds.")
hi_text = """
...
# print(f"Translation all at once")
# print(trans)
# print(f"Translation took {time.time() - start:.2f} seconds.")


# split input text into smaller sentences by punctuation
hi_text = hi_text.replace("。", "。\n").replace("！", "！\n").replace("？", "？\n").strip()

# loop through each sentence and translate

hi_text = hi_text.split("\n")
hi_text = [sentence.strip() for sentence in hi_text if sentence.strip()]
print(hi_text)
...
for i, sentence in enumerate(hi_text):
    ...
    generated_tokens = model.generate(**encoded_hi, forced_bos_token_id=tokenizer.get_lang_id("vi"))
    trans = tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)
    print(f"Translation {i + 1}/{len(hi_text)}")
    print(trans)
    print(f"Translation took {time.time() - start:.2f} seconds.")
